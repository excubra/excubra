  ...
  return (
    <Card className={d.findings > 0 ? "border-destructive/40" : ""}>
      ...
      <CardContent className="grid gap-2 text-sm">
        ...
        <div className="grid grid-cols-[140px_1fr] gap-3"><span className="text-muted-foreground">Heute</span><span>{today ? <>{today.Queries} Anfragen · {today.Blocked} geblockt · {today.NXDomain} unbekannte Namen{today.Failed ? <span className="text-destructive"> · {today.Failed} ohne Antwort vom Upstream</span> : null}{r?.refused ? <span className="text-muted-foreground"> · zuletzt {r.refused} von außerhalb des LANs verworfen</span> : null}</> : "–"}</span></div>
